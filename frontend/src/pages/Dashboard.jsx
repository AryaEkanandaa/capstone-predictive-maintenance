...
export default function Dashboard() {
  ...
  const handleSensorUpdate = useCallback(u=>{
    setMachines(prev=>prev.map(m=>{
      ...
      const chart=[...m.chartPoints,{x:new Date(t).getTime(),y:u.rotational_speed}].slice(-CHART_MAX_POINTS);
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
            {machines.filter(m=>m.status!=="NORMAL").map(m=>(
              <div key={m.machine_id} className={`p-4 rounded-lg shadow text-sm ${
                m.status==="CRITICAL"?"bg-red-50 border border-red-400":"bg-yellow-50 border border-yellow-400"}`}>
